...
void AnalysisManager::BeginOfRun(const G4Run *pRun)
{
	...
	if(fVerbose>=AnalysisVerbosity::kInfo) std::cout << "\nInfo --> AnalysisManager::BeginOfRun: Initialize random numbers with seed = " << randseed << std::endl;
	...
	fTreeFile = TFile::Open(fDataFilename.c_str(), "RECREATE", "File containing event data of MediPix simulation");
	...
	fTree = new TTree("t1", "Tree containing event data for MediPix simulations.");
	...
	fTree->Branch("prim_vol_globcp", &fEventData->fPrimaryVolumeGlobCp, "prim_vol_id/I");//Fill at start of Event
	...
	if(fSave >= DatasaveLevel::kSdSteps) fTree->Branch("ekin", "vector<Double_t>", &fEventData->fEkin);//Fill at step stage
...
void AnalysisManager::BeginOfEvent(const G4Event *pEvent)
{
	...
	//Here I can still use the tracking navigator as the tracking have not started yet
...
void AnalysisManager::EndOfEvent(const G4Event *pEvent)
{
	if(fSave>DatasaveLevel::kOff){
		...
		//This a temporary solution before understanding the reason some 
...
void AnalysisManager::Step(const G4Step *pStep, const G4SteppingManager* pStepMan)
{
	...
	} //Closes if(fVerbose>=AnalysisVerbosity::kDetails || fStepsDebug)
...
void AnalysisManager::DefineSensDet(G4String volList)
{
...
void AnalysisManager::MakeVolMaps()
{
	...
	const G4VPhysicalVolume* worldPhysVol = (dynamic_cast<const DetConstrMediPix*> (G4RunManager::GetRunManager()->GetUserDetectorConstruction()) )->GetWorldVolume();
...
{
	if(!aProcess) return -1;//This is an error return code
...
std::string AnalysisManager::BuildPysVolDict()
{
	std::string dictstr("");
...
int AnalysisManager::FindVolGlobalCopy(G4TouchableHandle& touch)
{
